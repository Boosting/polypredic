...
class Tool(object):
    def __init__(self, args):                   ## HTF the args given is defined here but not in the above object place
        self.opts = json.load(open(args.exp, 'r'))
        self.image_dir = args.image_dir
        self.data_loader = get_data_loaders(self.opts['dataset'], cityscapes.DataProvider)
        self.model = polyrnnpp.PolyRNNpp(self.opts).to(device)      ## Network defenition
        self.model.reload(args.reload, strict=False)                ## loading of .pth from the saved models folder
    ...
    def annotation(self, instance, run_ggnn=False):
        with torch.no_grad():   ## getting the poly from the img
    ...
    def fixing(self, instance, run_ggnn=False):     ## fixing the corrected preds
        with torch.no_grad():
            ...
            img = torch.from_numpy(img).to(device)
            print('Contents inside the fixing method: ', poly, type(poly))
...
def generate_annotation():
    ...
    print ("instance json inside annotation:",instance)
    component = {}
...
@app.route('/api/annotation_and_ggnn', methods=['POST'])        ## direct BB drawn and output comes Here
def generate_annotation_and_ggnn():
    start = time.time()
    instance = request.json
    print ("instance json inside annotation_and_ggnn:",instance)
    component = {}
    component['poly'] = np.array([[-1., -1.]])
    instance = tool.data_loader.prepare_component(instance, component)
    print ("instance json inside annotation_and_ggnn after prepare:",instance)
...
def fix_poly_request():
    start = time.time()
    instance = request.json
    print ("instance json inside fix_poly:",instance)
    component = {}
...
def fix_poly_ggnn_request():            ## Here the interaction of poly by the user and then the same is fed for correction to the model
    start = time.time()
    instance = request.json             # this is to get the values from the browser
    print ("instance json inside fix_poly_ggnn_request:",instance)      # instance contains the predicted coordinates now( I hope this one is with the corrected coordinates)
    component = {}
    component['poly'] = instance['poly']                                # component is loaded with the predicted coordinates
    instance = tool.data_loader.prepare_component(instance, component)
    pred_annotation = tool.fixing(instance, run_ggnn=True)
    pred_annotation[0] = pred_annotation[0][::2]

    print "Fixing time for fix_poly_ggnn: " + str(time.time() - start)
...
def ggnn_poly_request():
    start = time.time()
    instance = request.json
    print ("instance json inside ggn_poly_req:",instance)
...
def upload_v3():
    instance = request.json
    print ("instance json inside upload_v3:",instance)
...
def upload_v2():
    instance = request.json
    print ("instance json inside upload_v2:",instance)
